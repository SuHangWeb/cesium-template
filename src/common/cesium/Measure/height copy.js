 ...
 * 高度
 */
class Height extends MeasureCommon {
    positions = [];
    vertexEntities = [];
    labelEntity = undefined;
    measureHeight = 0; //测量结果
    activate() {
        this.deactivate();
        ...
        this.circleEntity = this.viewer.entities.add({
            ...
            ellipse: {
                height: new Cesium.CallbackProperty(e => {
                    const cartographic = this.viewer.scene.globe.ellipsoid.cartesianToCartographic(this.positions[this.positions.length - 1]);
