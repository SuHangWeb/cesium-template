 ...
 * 使用方法如下 
   const _Transform = new Transform(Cesium上下文,场景viewer)
    ...
    meter2Lat | 距离（米）转换为纬度  一米对应的纬度为定值
    colorRgb | 输入16进制颜色值 转 rgb 的数组值 255
    colorRgbString | 输入16进制颜色值 转 rgb 的字符 255
    colorRgb1 | 输入16进制颜色值 转 rgb 的数组值 0.0 - 1.0
    colorRGB2Hex | RGB转16进制(rgb2hex)
 */
class Transform {
     ...
     * @returns 
     */
    colorRgbString(inColor) {
        let arr = this.colorRgb(inColor)
     ...
     */
    colorRgb1(inColor) {
        const colorChange = this.colorRgb(inColor)
